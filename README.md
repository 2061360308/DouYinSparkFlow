...
首次使用请在登录页面点击“使用邀请码注册”。以下邀请码每个仅能成功注册一次，先到先得：

```text
d3kK3oYsQ3FQ7Z-NyCjIus_fhbu6C3Ae
rcXG2u2drc4Y_4OHRds3EWUTIUWLyB1q
Hkona8whrmGAn8FR5XukJuaRA9uIQTrD
IBw8-wnB_nckPNOsNlymCmt_us9MJotK
KKpv_Xn6alX3mS1pC3sbbrYyrjh9xD5I
```

### 操作步骤

1. 打开在线入口；尚未登录时，页面会先进入登录流程。
2. 点击“使用邀请码注册”，填写用户名、密码和一个尚未使用的邀请码。用户名须为 3–32 位字母、数字、下划线或短横线；密码至少 10 位，并同时包含字母和数字。
3. 注册成功后登录控制台，进入“抖音账号”页面，点击“扫码绑定抖音账号”。如抖音要求短信验证，请按页面提示输入手机收到的验证码。
4. 绑定成功后进入“续火任务”，选择发送账号并读取最近会话；从列表选择目标好友，也可以手动输入昵称。
5. 设置每天发送时间和消息内容，点击“创建并启用任务”。
6. 在“执行记录”中查看每次任务的发送时间、执行阶段和结果；需要调整时可在任务页面编辑、暂停或删除任务。
