...
本仓库新增了一个独立的 FastAPI 控制台：受邀用户注册后可通过抖音二维码绑定账号，设置目标好友、北京时间、发送内容并查看脱敏执行结果；管理员可以生成一次性邀请码、停用用户，并查看、暂停和删除所有任务。

- 扫码进程只把 AES-256-GCM 加密后的登录状态写入数据库；当前二维码会短暂展示，明文凭证不会经过 Web 服务。
- SQLite 使用 WAL，Worker 全局单并发，同一次计划只执行一次，失败不自动重发。
- Docker Compose 仅让 `spark-web` 绑定 `127.0.0.1:8899`；`spark-auth` 不发布端口、不挂载 Docker socket，并与控制台服务共用独立私有网络和数据卷。
- 正式给朋友使用前必须配置可信域名和 HTTPS。不得直接开放 8899 端口或通过明文 HTTP 传递账号凭据。
- 部署前要求根分区至少剩余 5 GiB，并确认系统时间误差不超过 5 秒。

二维码与邀请功能升级时，先备份 SQLite，只构建并重建 `spark-web` 和 `spark-auth`。此次升级不启动或重建 `spark-worker`，也不连接或改动 BPS；完整命令和保留数据的回滚步骤见运维手册。
